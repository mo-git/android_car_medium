    ...
    public void setData(JoinUserBean.JoinBean bean){
        ...
        if(bean.getDescribe() != null){
            ...
            if(!CollectionUtils.isEmpty(bean.getDescribe().getPics())){
                ImageLoader.getInstance().displayImage(bean.getDescribe().getPics().get(0),ImageUtils.getImageViewAware(iv_icon), ImageUtils.loadImage(R.drawable.defal_image));
            }
        }
        ...
        if(bean.getLuck() != null && !CollectionUtils.isEmpty(bean.getLuck())){
            ...
            for (JoinUserBean.LuckbeBean lubkBean : bean.getLuck()) {
                ...
                ImageLoader.getInstance().displayImage(lubkBean.getUser().getAvatar_url(),ImageUtils.getImageViewAware(imageView), ImageUtils.loadImage(R.drawable.defal_image));
